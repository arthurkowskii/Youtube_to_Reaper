-- YouTube → New Track v1.1 Beta A
-- Option A: Enhanced Console Feedback for progress indication
...
reaper.ShowConsoleMsg("✓ Tools ready: yt-dlp + ffmpeg found\n")
...
reaper.ShowConsoleMsg("✓ Starting download...\n")
reaper.ShowConsoleMsg("  → This may take 30-90 seconds depending on video length and connection\n")
reaper.ShowConsoleMsg("  → Please wait, the script is working...\n")

-- Execute download command with extended timeout for progress indication
local start_time = reaper.time_precise()
local result = reaper.ExecProcess(cmd, 120000) -- Extended to 2 minutes
local elapsed_time = reaper.time_precise() - start_time

-- Verify output file exists and has content
if not reaper.file_exists(output_file) then
    reaper.ShowConsoleMsg("✗ ERROR: Download failed after " .. string.format("%.1f", elapsed_time) .. " seconds\n")
    reaper.ShowConsoleMsg("  → Check your internet connection and try again\n")
    if result then
        reaper.ShowConsoleMsg("  → Details: " .. result .. "\n")
    end
    return
end
...
reaper.ShowConsoleMsg("✓ Download completed in " .. string.format("%.1f", elapsed_time) .. " seconds (" .. format_size(file_size) .. ")\n")
reaper.ShowConsoleMsg("✓ Importing audio to new track...\n")
...
if insert_result > 0 then
    ...
    reaper.ShowConsoleMsg("  → Total time: " .. string.format("%.1f", elapsed_time) .. " seconds\n")
else
    reaper.ShowConsoleMsg("✗ ERROR: Failed to import audio file to REAPER\n")
end